...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!name || !email || !message) {
      console.error("Missing required fields", { name: name ? "present" : "missing", email: email ? "present" : "missing", message: message ? "present" : "missing" });
      return new Response(
    ...
    console.log(`Attempting to send contact email from ${name} (${email})`);
    ...
    console.log("Contact email send response:", JSON.stringify(emailResponse));
