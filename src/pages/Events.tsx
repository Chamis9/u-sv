        ...
        <main className="flex-grow pt-24 pb-12">
          <div className="container mx-auto px-4">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 animate-pulse">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="h-48 bg-teal-600 rounded-lg"></div>
              ))}
            </div>
          </div>
        </main>
              ...
              {categories?.map((category) => {
                ...
                return (
                  <Link to={`/events/${slug}`} key={category.id}>
                    <Card className="h-full transition-transform hover:scale-105 bg-teal-600/50 backdrop-blur-sm border border-cream/30">
                      <CardHeader>
